...
## Later standalone EPSC manuscript
...
This manuscript assembles the later EPSC results but does not retroactively modify the deposited v0.1.0 paper.
...
| K=5 therefore contains all continuum NS information | **No. HOLD.** Nested agreement and boundary energy do not bound the complete omitted tail |
...
| Raw values from the current finite Galerkin solver may be substituted directly into the continuum energy-budget theorem | **No. HOLD.** A certified adapter is required |
| Residual-based relative-energy estimate `sup ||u-v||_2^2 <= exp(A_T)(e0^2+B_T/nu)` for a smooth divergence-free comparison path | **Standard analytic (`Dr`) estimate specialized to EPSC.** Under the declared assumptions it supplies a terminal continuum bound when `v(T)` is K-supported |
| The unresolved nonlinear residual of an exact finite Fourier-Galerkin comparison path is finite-computable as a triad tape, including its snapshot H^-1 norm | **Finite algebra/definition with finite checker support.** This does not by itself certify its continuous-time integral |
| The current floating-point RK4 tape already has a rigorous continuous-time enclosure of `A_T` and `B_T` | **OPEN.** Toledo `PROP-EPSC-15`. Nodewise residual samples or backend agreement are insufficient |
| The EPSC family is registered in Toledo | **Yes, as proposals rather than canonical verified theorem codes, through `PROP-EPSC-15`.** |
| The EPSC results solve the Clay Millennium problem | **No.** They neither prove global smoothness nor finite-time blow-up, and do not establish uniqueness of 3-D weak solutions |
| The short refinement result overturns the prior negative coarse Taylor-Green turbulent validation | **No.** Nested short-horizon stability and external physical/DNS adequacy are different claims |

## Current end-to-end frontier

```text
floating-point RK4 tape
    -> PROP-EPSC-15 validated continuous-time interpolation/enclosure (OPEN)
    -> certified A_bar_T and B_bar_T
    -> PROP-EPSC-13 relative-energy adapter
    -> terminal continuum L2 beta_K
    -> fail-closed epsilon certificate
```

The key methodological rule is unchanged: **finite algebra / nested stability does not by itself imply continuum completion, and continuum completion does not by itself imply physical turbulence validation.**
...
Usage:
    python .github/clay_governance_gate.py <base_sha> <head_sha>

The gate is intentionally lightweight. It does not prove that a cross-repo
Toledo update actually happened; it requires an auditable acknowledgement in
CLAY_GOVERNANCE_ACK.json, and it rejects high-impact changes when Toledo is
marked not-required.
"""
...
PROFILES = {
    "morrocwi/readout-problem-navier-stokes": {
        "sensitive_exact": {
            ...
            "CLAY_MULTI_PROBLEM_FINITE_BRIDGE_PROGRAM.md",
        },
    ...
    "morrocwi/information-discrete-math": {
        "sensitive_exact": {
            "README.md",
            "AGENTS.md",
            "docs/UNIVERSAL_FINITE_OBSTRUCTION_UNIFORM_BRIDGE_KERNEL.md",
            "docs/DISCRETE_EPSILON_COMPLETION.md",
            "docs/FINITE_DIRECT_SAMPLE_BRANCH.md",
            "docs/FORMAL_COMPANIONS.md",
        },
    ...
    "morrocwi/toledo": {
        "sensitive_exact": {
            "README.md",
            "AGENTS.md",
            "docs/CLAY_BRIDGE_PROGRAM_2026-09-11.md",
            "docs/EQ_CODE_SCHEME.md",
            "registry/SCHEMA.md",
            "registry/CANONICAL.json",
            "registry/CANONICAL_REGISTRY.json",
            "registry/CANONICAL_MAP.md",
            "registry/EQ_LIBRARY.md",
        },
...
def main() -> None:
    ...
    sensitive = [
        ...
        if matches(
            p,
            profile["sensitive_exact"],
            profile["sensitive_prefixes"],
            profile["sensitive_contains"],
        )
    ]
    ...
    high = [
        ...
        if matches(
            p,
            profile["high_exact"],
            profile["high_prefixes"],
            profile["high_contains"],
        )
    ]
    ...
    if ACK not in files:
        hold(
            f"{ACK} must be updated in the same PR when Clay-sensitive paths change"
        )

    ack_path = Path(ACK)
    if not ack_path.exists():
        hold(f"{ACK} is missing from the checked-out head")
    ...
    if high:
        if toledo_status == "not-required":
            hold(
                "high-impact claim/formal/bridge changes require Toledo status 'updated' "
                "or 'issue-open', not 'not-required'"
            )
        if claim_effect in {"none", "documentation-only"}:
            hold(
                "high-impact paths require claim_effect to acknowledge a status, statement, "
                "formal-proof, or provenance change"
            )

...
while the gradient integral is bounded by a rigorous coefficientwise Fourier majorant.  Executable
...
\item \texttt{readout\_genesis}: interpretation/application map only; NS results are not promoted to
      root ontology.
\end{itemize}
The EPSC lineage is registered as \texttt{PROP-EPSC-01} through \texttt{PROP-EPSC-19}.  Classical
...
a certified comparison path into a terminal $L^2$ bound.  The declared stored-RK4-tape construction
now supplies a rigorous continuous-time enclosure for that path.  The remaining outer numerical
frontier is tightness and computational scaling at larger cutoff/horizon (\texttt{PROP-EPSC-16});
measurement-to-continuum composition with energy observability is treated separately in
\texttt{NS\_OBSERVABLE\_TO\_CONTINUUM\_CERTIFICATES.tex}.
...
The continuous-time residual enclosure of the declared stored RK4 tape is now supplied by the
exact-dyadic piecewise-linear construction.  The remaining outer implementation frontier is making
that certification tight and affordable at larger $K$ and longer $T$ (\texttt{PROP-EPSC-16}).
A companion synthesis paper separates this outer problem from the certified energy-observability
inverse and noise-stability frontiers (\texttt{PROP-EPSC-18/19}).
